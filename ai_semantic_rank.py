#!/usr/bin/env python3
"""
AI-Powered Semantic Rank Game CSV Generator

This script generates a precomputed CSV file for the Semantic Rank word game
using AI to write individual clues for each word relative to the secret word.
"""
...
class AISemanticRankGenerator:
    def __init__(self, secret_word="planet", use_ai=True):
        ...
        if self.use_ai:
            ...
            try:
                ...
                if not api_key:
                    ...
                    print("AI clue generation will be disabled. Using pattern-based clues instead.")
                    self.use_ai = False
                else:
                    self.client = OpenAI(api_key=api_key)
                    print("AI clue generation enabled!")
            except Exception as e:
                print(f"Warning: Could not initialize OpenAI client: {e}")
                self.use_ai = False
        
        if not self.use_ai:
            print("Using pattern-based clue generation (no AI).")
    ...
    def generate_ai_clue(self, word, rank):
        """Generate a clue using AI based on the word's relationship to the secret word."""
        if not self.use_ai:
            return self.generate_fallback_clue(word, rank)
...
Clue:"""
            ...
            print(f"AI clue generation failed for '{word}': {e}")
            return self.generate_fallback_clue(word, rank)
    
    def generate_fallback_clue(self, word, rank):
        """Generate a fallback clue when AI is not available or fails."""
        if rank == 1:
            return "This is the *."
        elif rank <= 1000:
            patterns = [
    ...
    def generate_csv(self, output_filename="ai_semantic_rank_data.csv", batch_size=50):
        """Generate the complete CSV file with AI-generated clues."""
        print(f"Generating CSV file: {output_filename}")
        
        # Prepare data
        csv_data = []
        
        print(f"Generating {'AI-powered' if self.use_ai else 'pattern-based'} clues for all words...")
        ...
        for i, word in enumerate(self.words):
            ...
            # Add small delay for AI requests to avoid rate limits
            if self.use_ai and i % batch_size == 0 and i > 0:
                time.sleep(1)  # Brief pause every batch_size words
    ...
    def run_full_generation(self, output_filename="ai_semantic_rank_data.csv"):
        """Run the complete generation process."""
        print("=== AI-Powered Semantic Rank CSV Generator ===")
        print(f"Secret word: {self.secret_word}")
        print(f"AI clue generation: {'Enabled' if self.use_ai else 'Disabled (using patterns)'}")
        print()
...
def main():
    ...
    generator = AISemanticRankGenerator(secret_word, use_ai=True)
    
    output_file = f"ai_semantic_rank_{secret_word}.csv"
    ...
    if success:
        ...
        if generator.use_ai:
            print("✨ Each clue was individually crafted by AI!")
        else:
            print("📝 Clues generated using pattern-based system.")
    else:
