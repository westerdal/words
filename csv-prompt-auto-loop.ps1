...
<#
.SYNOPSIS
    Automated CSV-Prompt Loop - Process all remaining words
    
.DESCRIPTION
    Runs the csv-prompt process in a loop until all words are processed
#>
...
function Reserve-Word($word) {
    $timestamp = (Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ')
    Add-Content $lockFile "$word $timestamp"
    return $timestamp
}

function Release-Word($word) {
    $content = Get-Content $lockFile
    $cleanedContent = $content | Where-Object { $_ -notmatch "^$word\s+" }
...
for ($i = 1; $i -le $MaxIterations; $i++) {
    ...
    # Reserve the word
    $reserveTime = Reserve-Word $nextWord
    Write-Host "✅ Reserved: $nextWord"
    
    try {
        # Generate CSV
        Write-Host "🚀 Generating CSV for '$nextWord'..."
        $result = python scripts\processing\generate_csv.py $nextWord 2>&1
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "📊 Progress: $completedCsvs/$totalEmbeddings ($progress%)" -ForegroundColor Cyan
        } else {
...
if ($needCsv.Count -eq 0) {
    ...
    Write-Host ""
    Write-Host "⏸️ Loop stopped with $($needCsv.Count) words remaining" -ForegroundColor Yellow
    Write-Host "Next words to process: $($needCsv | Select-Object -First 5 | Join-String -Separator ', ')"
}
